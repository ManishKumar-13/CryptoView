...
function loadDetail() {
    ...
    fetch(`https://api.coingecko.com/api/v3/coins/${params.get('id')}?calization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false`)
    .then(convertToJSON)
    .then(render);
  
    fetch(`https://api.coingecko.com/api/v3/coins/${params.get('id')}/market_chart?vs_currency=inr&days=1`)
    .then(convertToJSON)
    .then(renderChart);
  }
